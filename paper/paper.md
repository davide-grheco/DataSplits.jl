...
Within Julia, MLUtils.jl provides an observation interface based on `numobs`, `getobs`, and lazy observation views, together with data loaders and basic splitting utilities [@mlutils]. Flux.jl re-exports MLUtils data-loading functionality for neural network training [@Innes2018Flux]. MLJ.jl provides a comprehensive model-evaluation framework with built-in holdout and cross-validation resampling strategies, and also accepts explicit train/test row-pair vectors for custom resampling workflows [@Blaom2020MLJ]. None of these three implement distance-based, group-stratified, or purged time-series splitting strategies.
The closest Julia package in scope is ChemometricsTools.jl [@chemometricstools], a broad chemometrics toolkit offering Kennard--Stone selection, two-way `SplitByProportion` and `VenetianBlinds` splits, and `KFoldsValidation`/`LeaveOneOut` iterators, all returning materialised arrays. To our knowledge, no Julia package implements group-stratified or purged time-series splitting.
