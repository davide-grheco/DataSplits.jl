...
@testset "StratifiedShuffleSplit" begin
  @testset "Classification — class proportions preserved per resample" begin
    ...
    for f in folds(cvs)
      @test has_correct_split_size(f, 80, 20)
      @test is_disjoint(f)
      @test is_full_partition(f, 100)
  ...
  @testset "Regression — quantile binning" begin
    ...
    cvs = partition(
      X,
      StratifiedShuffleSplit(5; bins = 4);
      target = y,
      train = 0.7,
      test = 0.3,
    ...
    for f in folds(cvs)
      @test has_correct_split_size(f, 70, 30)
      @test is_disjoint(f)
    end
  end
  ...
  @testset "n_splits and bins validation" begin
    ...
    @test_throws SplitParameterError partition(
      rand(2, 100),
      StratifiedShuffleSplit(3; bins = 1);
      target = randn(100),
      train = 0.8,
      test = 0.2,
    )
