@testitem "KennardStoneSplit (CADEX)" begin
  using Random
  using Distances
  using DataSplits
  using Test
  ...
  X = rand(50, 3)
  strat = KennardStoneSplit(0.8, Euclidean())
  ...
  Xv = [X[i, :] for i in 1:size(X, 1)]
  tr2, te2 = DataSplits.split(Xv, KennardStoneSplit(0.3, CosineDist()); rng=Random.GLOBAL_RNG)
  ...
  tr1, te1 = DataSplits.split(X, KennardStoneSplit(0.25, Euclidean()); rng=rng1)
  rng2 = MersenneTwister(123)
  tr2b, te2b = DataSplits.split(X, KennardStoneSplit(0.25, Euclidean()); rng=rng2)
  @test tr1 == tr2b && te1 == te2b

  # Test 4: frac boundary handling
  stratf = KennardStoneSplit(0.02, Euclidean())
  @test_throws ArgumentError trf, tef = DataSplits.split(rand(10, 2), stratf)

  stratf = KennardStoneSplit(0.98, Euclidean())
  @test_throws ArgumentError trf, tef = DataSplits.split(rand(10, 2), stratf)

  # Test 5: edge-case of too-small frac
  @test_throws ArgumentError KennardStoneSplit(0.0, Euclidean())
  @test_throws ArgumentError DataSplits.split(rand(5, 5), KennardStoneSplit(1.0, Euclidean()))

  # Test 6: expected max-min property (somewhat)
  X2 = vcat(randn(5, 2) .+ 5, randn(5, 2) .- 5)
  tr_small, te_small = DataSplits.split(X2, KennardStoneSplit(0.2, Euclidean()))
